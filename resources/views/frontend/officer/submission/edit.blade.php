...
@section('content')
    ...
    <main class="flex-1 bg-gray-50 p-8">
        ...
        <div class="bg-gray-200 shadow rounded-lg p-8">
          ...
          <form action="{{ route('officer.submissions.update', ['id'=>$submission->id]) }}" method="POST" class="grid grid-cols-2 gap-6">
            @method('PUT')
            @csrf
            <div>
              <label for="judul" class="block text-sm font-medium text-gray-700">Judul</label>
              <input id="title" name="title" value="{{ $submission->title }}" type="text" placeholder="Input judul" class="mt-1 w-full border border-gray-300 rounded-lg shadow-sm focus:ring-[#124868] focus:border-[#124868] px-4 py-2" >
            </div>
            <div>
              ...
              <div class="relative inline-block w-64 mb-4">
                ...
                <div class="dropdownMenu absolute hidden mt-2 w-full bg-white border border-gray-300 rounded-md shadow-lg">
                  <div class="p-2">
                    @php
                        function CheckIfExists($domain, $submission)
                        {
                            [$type, $id] = explode("_", $domain);
                            if($submission->submission_domains->where('domain_type', $type)->where('domain_id', $id)->count() > 0)
                                return true;
                    ...
                    @if (isset($domains)&&$domains!=null&&$domains->count() > 0)
                        @foreach ($domains as $item)
                            <label class="flex items-center">
                                <input type="checkbox" value="{{ $item }}" name="domains[]" class="mr-2" @checked(CheckIfExists($item, $submission)) > {{ $item }}
                            </label>
                        @endforeach
                    @else
                    @endif
            ...
            <div class="col-span-2">
              ...
              <textarea id="desc" name="desc" placeholder="Deskripsi submission" class="mt-1 w-full border border-gray-300 rounded-lg shadow-sm focus:ring-[#124868] focus:border-[#124868] px-4 py-2" rows="3">{{ $submission->desc }}</textarea>
            </div>
            <div>
              <label for="deadline" class="block text-sm font-medium text-gray-700">Deadline</label>
              <input id="deadline" name="deadline" type="datetime-local" class="mt-1 w-full border border-gray-300 rounded-lg shadow-sm focus:ring-[#124868] focus:border-[#124868] px-4 py-2" value="{{ $submission->deadline }}">
            </div>
            <div>
                      ...
                      <div class="p-2">
                        @if (isset($document_types)&&$document_types!=null&&$document_types->count() > 0)
                            @foreach ($document_types as $item)
                                <label class="flex items-center">
                                    <input type="checkbox" value="{{ $item->id }}" name="document_requirements[]" class="mr-2" @checked($submission->document_requirements->where('type_id', $item->id)->count() > 0)> {{ $item->name }}
                                </label>
                            @endforeach
                        @else
